...
namespace myers {
    ...
    public:
        ...
        Path(int fr_x, int fr_y, int to_x, int to_y)
        : fr_x(fr_x), fr_y(fr_y), to_x(to_x), to_y(to_y) {};
    };
    ...
    template <typename T>
    class Diff {
        std::vector<Path> backtrack(const std::vector<T>& a,
                               const std::vector<T>& b);
        std::vector<NegIndVector<int>> shortest_edit_path(const std::vector<T>& a,
                               const std::vector<T>& b);

    public:
        using Comparator = std::function<bool(const T&, const T&)>;

        Diff(Comparator comp = std::equal_to<T>())
            : comp_(comp) {}

        std::vector<Edit<T>> diff(const std::vector<T>& a,
                               const std::vector<T>& b);
